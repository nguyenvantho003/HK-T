...
const getItemById = (req, res) => {
  ...
  if(!checkItem) {
    handleResponseSuccess(res,404, "Item not found")
    return
  }
...
const getItemsPagination = (req, res) => {
  ...
  handleResponseSuccess(res, 200, "Get items successfully", { ...result })
}
...
const deleteItem = (req, res) => {
  const { id } = req.params
  const checkedItemIndex = items.findIndex(items => item.id === parseInt(id))
